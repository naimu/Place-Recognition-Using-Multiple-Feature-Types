 ...
 * Description: functions for LBD descriptors
...
void FLBD::meanValue(const std::vector<FLBD::pDescriptor> &descriptors, 
  FLBD::TDescriptor &mean)
{
  ...
  else
  {
    ...
    mean = cv::Mat::zeros(1, FLBD::L, CV_8U);
...
double FLBD::distance(const FLBD::TDescriptor &a, 
  const FLBD::TDescriptor &b)
{
...
std::string FLBD::toString(const FLBD::TDescriptor &a)
{
  stringstream ss;
...
void FLBD::fromString(FLBD::TDescriptor &a, const std::string &s)
{
  a.create(1, FLBD::L, CV_8U);
  unsigned char *p = a.ptr<unsigned char>();
  
  stringstream ss(s);
  for(int i = 0; i < FLBD::L; ++i, ++p)
  {
    int n;
    ss >> n;
...
void FLBD::toMat32F(const std::vector<TDescriptor> &descriptors, 
  cv::Mat &mat)
{
  if(descriptors.empty())
  {
...
void FLBD::toMat32F(const cv::Mat &descriptors, cv::Mat &mat)
{
...
void FLBD::toMat8U(const std::vector<TDescriptor> &descriptors, 
  cv::Mat &mat)
{
  mat.create(descriptors.size(), FLBD::L, CV_8U);
  
  unsigned char *p = mat.ptr<unsigned char>();
  
  for(size_t i = 0; i < descriptors.size(); ++i, p += FLBD::L)
  {
    const unsigned char *d = descriptors[i].ptr<unsigned char>();
    std::copy(d, d + FLBD::L, p);
  }
