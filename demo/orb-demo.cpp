...
void loadFeatures(vector<vector<Mat> > &features, string dir, int startNumber, int imgNumber)
{
  ...
  char fileName[256];
  cout << "Extracting orb features from: "<< dir << endl;
...
//    ss << "images/image" << i << ".png";
    ...
    imgDescVec.resize(descriptors.rows);
    ...
    for(size_t j = 0; j < imgDescVec.size(); ++j) {
        ...
        imgDescVec[j] = descriptors.row(j);
        //if(i == 0 && j == 0)
    ...
    if(i % 100 == 0)
        printf("loaded features for image %s with %lu keypoints\n", imgName.c_str(), imgDescVec.size());
    //printf("========== frame %d ===========\n", i);
...
void createVoc(const vector<vector<Mat> > &features, int imageNumber, int vocBranchNumber, int vocLevelNumber, string vocFileName)
{
        ...
        case CREATEALL:
            {
                ...
                createVoc(features, totalImageNumber, vocBranchNumber, vocLevelNumber, vocFileName);
